...
if (isset($_SESSION['user_id'])) {
    ...
    $check_notifications = pg_query_params($dbconn, "SELECT unread_notifications FROM users WHERE id = $1", [$user_id]);
...
function is_banned() {
    ...
    $user_id = (int) $_SESSION['user_id'];
    $query = pg_query_params($dbconn, "SELECT is_banned FROM users WHERE id = $1", [$user_id]);
...
function logout_user() {
    ...
    if (isset($_SESSION['user_id'])) {
        $user_id = (int) $_SESSION['user_id'];
        pg_query_params($dbconn, "UPDATE users SET remember_token = NULL, token_expires = NULL WHERE id = $1", [$user_id]);
    }
