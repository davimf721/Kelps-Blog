...
if ($is_admin) {
    ...
    $query = "SELECT c.*, p.id as post_id FROM comments c 
              ...
              WHERE c.id = $1";
    $result = pg_query_params($dbconn, $query, [$comment_id]);
} else {
    // Usuário comum só pode excluir seus próprios comentários
    $query = "SELECT c.*, p.id as post_id FROM comments c 
              JOIN posts p ON c.post_id = p.id 
              WHERE c.id = $1 AND c.user_id = $2";
    $result = pg_query_params($dbconn, $query, [$comment_id, $user_id]);
}
...
if ($is_admin) {
    $delete_result = pg_query_params($dbconn, "DELETE FROM comments WHERE id = $1", [$comment_id]);
} else {
    $delete_result = pg_query_params($dbconn, "DELETE FROM comments WHERE id = $1 AND user_id = $2", [$comment_id, $user_id]);
}
