...
// Buscar estatísticas do usuário usando prepared statement
$stats_query = "
    SELECT 
        ...
        (SELECT COALESCE(SUM(upvotes_count), 0) FROM posts WHERE user_id = $1) as total_upvotes
";

// Executar query com verificação de erro
$stats_result = pg_query_params($dbconn, $stats_query, [$user_id]);
if (!$stats_result) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'request_deletion') {
        ...
        if (empty($password)) {
            ...
            // Verificar senha
            $password_result = pg_query_params($dbconn, "SELECT password_hash FROM users WHERE id = $1", [$user_id]);
...
function delete_user_account($dbconn, $user_id, $username) {
    ...
    try {
        ...
        if (in_array('user_logs', $existing_tables)) {
            $log_result = pg_query_params($dbconn, 
                "INSERT INTO user_logs (user_id, action, ip_address, user_agent, created_at) VALUES ($1, 'account_deleted', $2, $3, NOW())",
                [$user_id, $_SERVER['REMOTE_ADDR'] ?? 'unknown', $_SERVER['HTTP_USER_AGENT'] ?? 'unknown']
            );
        ...
        // 3. Deletar relacionamentos em ordem segura usando prepared statements
        
        // Deletar seguidores/seguindo (se a tabela existir)
        if (in_array('followers', $existing_tables)) {
            $result = pg_query_params($dbconn, "DELETE FROM followers WHERE follower_id = $1 OR following_id = $1", [$user_id]);
            if (!$result) {
                throw new Exception("Erro ao deletar followers: " . pg_last_error($dbconn));
            }
        }
        
        // Deletar upvotes do usuário (se a tabela existir)
        if (in_array('upvotes', $existing_tables)) {
            $result = pg_query_params($dbconn, "DELETE FROM upvotes WHERE user_id = $1", [$user_id]);
            if (!$result) {
                throw new Exception("Erro ao deletar upvotes: " . pg_last_error($dbconn));
            }
        }
        ...
        if (in_array('notifications', $existing_tables)) {
            ...
            if ($check_column && pg_num_rows($check_column) > 0) {
                $result = pg_query_params($dbconn, "DELETE FROM notifications WHERE user_id = $1 OR from_user_id = $1", [$user_id]);
            } else {
                $result = pg_query_params($dbconn, "DELETE FROM notifications WHERE user_id = $1", [$user_id]);
            }
            if (!$result) {
                throw new Exception("Erro ao deletar notifications: " . pg_last_error($dbconn));
            }
        }
        
        // Deletar comentários (se a tabela existir)
        if (in_array('comments', $existing_tables)) {
            $result = pg_query_params($dbconn, "DELETE FROM comments WHERE user_id = $1", [$user_id]);
            if (!$result) {
                throw new Exception("Erro ao deletar comments: " . pg_last_error($dbconn));
            }
        }
        
        // Deletar posts (se a tabela existir)
        if (in_array('posts', $existing_tables)) {
            $result = pg_query_params($dbconn, "DELETE FROM posts WHERE user_id = $1", [$user_id]);
            if (!$result) {
                throw new Exception("Erro ao deletar posts: " . pg_last_error($dbconn));
            }
        }
        
        // Deletar perfil (se a tabela existir)
        if (in_array('user_profiles', $existing_tables)) {
            $result = pg_query_params($dbconn, "DELETE FROM user_profiles WHERE user_id = $1", [$user_id]);
            if (!$result) {
                throw new Exception("Erro ao deletar user_profiles: " . pg_last_error($dbconn));
            }
        }
        
        // 4. Deletar usuário (deve ser por último)
        $delete_user = pg_query_params($dbconn, "DELETE FROM users WHERE id = $1", [$user_id]);
